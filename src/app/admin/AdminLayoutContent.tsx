...
export default function AdminLayoutContent({ children, user }: { children: React.ReactNode, user: User | null }) {
  return (
    <div className="flex min-h-screen w-full bg-slate-100/50 dark:bg-[#04060a] text-slate-900 dark:text-gray-100 transition-colors duration-300">
      <AdminSidebar className="hidden md:flex w-[260px] lg:w-[280px] shrink-0" />
      <div className="flex flex-col flex-1 min-w-0 overflow-hidden">
        <AdminHeader user={user} />
        <main className="flex-1 overflow-y-auto p-4 md:p-6 lg:p-8 custom-scrollbar relative bg-slate-50 dark:bg-transparent">
